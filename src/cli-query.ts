...
import { parseConversation, parseFileHistory, getFileContent, exportSessionMarkdown, scanForSecrets } from './server/adapters/claude/parser.js';
import type { Session, SecretFinding } from './server/adapters/types.js';
...
export async function cmdScan(args: string[]): Promise<void> {
  ...
  const findings = await scanForSecrets(session.filePath);
